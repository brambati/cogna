...
if ($database) {
    try {
        $stats['total_tasks'] = $database->count('tasks', ['user_id' => $user_id]);
        $stats['completed_tasks'] = $database->count('tasks', ['user_id' => $user_id, 'status' => 'completed']);
        $stats['in_progress_tasks'] = $database->count('tasks', ['user_id' => $user_id, 'status' => 'in_progress']);
        $stats['pending_tasks'] = $database->count('tasks', ['user_id' => $user_id, 'status' => 'pending']);
        $stats['overdue_tasks'] = $database->count('tasks', [
            'user_id' => $user_id,
            'due_date[<]' => date('Y-m-d H:i:s'),
            'status[!]' => ['completed', 'cancelled']
        ]);
        $stats['total_categories'] = $database->count('task_categories', [
            'user_id' => $user_id,
            'is_active' => 1
        ]);
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $database && $user) {
    ...
    if ($action === 'update_profile') {
        ...
        if (empty($first_name)) {
            ...
            try {
                ...
                $existing_user = $database->get('users', 'id', [
                    'email' => $email,
                    'id[!]' => $user_id
                ]);
        ...
        if (empty($current_password)) {
            ...
            try {
                ...
                $user_password = $database->get('users', 'password_hash', ['id' => $user_id]);
