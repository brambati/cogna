<?php

/**
 * API Tasks - FluentPDO
 */

require_once __DIR__ . '/../../app/helpers/security.php';
require_once __DIR__ . '/../../app/models/User.php';
require_once __DIR__ . '/../../app/models/Task.php';

// Headers CORS e JSON
header('Content-Type: application/json; charset=utf-8');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: GET, POST, PUT, DELETE, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With');

// Handle preflight OPTIONS request
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit();
}

try {
    // Verificar autenticação
    $user = verifyApiAuthentication();
    if (!$user) {
        http_response_code(401);
        echo json_encode(['success' => false, 'errors' => ['Não autorizado']]);
        exit();
    }
    
    $taskModel = new Task();
    $method = $_SERVER['REQUEST_METHOD'];
    $path = $_SERVER['PATH_INFO'] ?? '';
    
    // Parse da URL para obter ID da tarefa
    $pathParts = explode('/', trim($path, '/'));
    $taskId = isset($pathParts[0]) && is_numeric($pathParts[0]) ? (int)$pathParts[0] : null;
    
    switch ($method) {
        case 'GET':
            if ($taskId) {
                // GET /api/tasks/{id} - Buscar tarefa específica
                $task = $taskModel->findById($taskId, $user['user_id']);
                if ($task) {
                    echo json_encode(['success' => true, 'task' => $task]);
                } else {
                    http_response_code(404);
                    echo json_encode(['success' => false, 'errors' => ['Tarefa não encontrada']]);
                }
            } else {
                // GET /api/tasks - Listar tarefas
                $filters = [
                    'status' => $_GET['status'] ?? '',
                    'priority' => $_GET['priority'] ?? '',
                    'search' => $_GET['search'] ?? '',
                    'due_date_from' => $_GET['due_date_from'] ?? '',
                    'due_date_to' => $_GET['due_date_to'] ?? ''
                ];
                
                $result = $taskModel->getByUser($user['user_id'], $filters);
                echo json_encode($result);
            }
            break;
            
        case 'POST':
            // POST /api/tasks - Criar nova tarefa
            $input = json_decode(file_get_contents('php://input'), true);
            
            // Verificar CSRF token
            if (!validateCSRFToken($input['csrf_token'] ?? '')) {
                http_response_code(403);
                echo json_encode(['success' => false, 'errors' => ['Token CSRF inválido']]);
                break;
            }
            
            $result = $taskModel->create($input, $user['user_id']);
            
            if ($result['success']) {
                http_response_code(201);
            } else {
                http_response_code(400);
            }
            
            echo json_encode($result);
            break;
            
        case 'PUT':
            // PUT /api/tasks/{id} - Atualizar tarefa
            if (!$taskId) {
                http_response_code(400);
                echo json_encode(['success' => false, 'errors' => ['ID da tarefa é obrigatório']]);
                break;
            }
            
            $input = json_decode(file_get_contents('php://input'), true);
            
            // Verificar CSRF token
            if (!validateCSRFToken($input['csrf_token'] ?? '')) {
                http_response_code(403);
                echo json_encode(['success' => false, 'errors' => ['Token CSRF inválido']]);
                break;
            }
            
            $result = $taskModel->update($taskId, $input, $user['user_id']);
            
            if ($result['success']) {
                echo json_encode($result);
            } else {
                http_response_code(400);
                echo json_encode($result);
            }
            break;
            
        case 'DELETE':
            // DELETE /api/tasks/{id} - Deletar tarefa
            if (!$taskId) {
                http_response_code(400);
                echo json_encode(['success' => false, 'errors' => ['ID da tarefa é obrigatório']]);
                break;
            }
            
            $result = $taskModel->delete($taskId, $user['user_id']);
            
            if ($result['success']) {
                echo json_encode($result);
            } else {
                http_response_code(400);
                echo json_encode($result);
            }
            break;
            
        default:
            http_response_code(405);
            echo json_encode(['success' => false, 'errors' => ['Método não permitido']]);
            break;
    }
    
} catch (Exception $e) {
    error_log("Erro na API de tarefas: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'errors' => ['Erro interno do servidor']]);
}

/**
 * Verificar autenticação da API
 */
function verifyApiAuthentication(): ?array {
    // Tentar autenticação por sessão primeiro
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }
    
    if (isset($_SESSION['user_id']) && isset($_SESSION['session_token'])) {
        $userModel = new User();
        $sessionData = $userModel->verifySession($_SESSION['session_token']);
        
        if ($sessionData) {
            return [
                'user_id' => $sessionData['user_id'],
                'username' => $sessionData['username'],
                'email' => $sessionData['email']
            ];
        }
    }
    
    // Tentar autenticação por API token
    $headers = getallheaders();
    $authHeader = $headers['Authorization'] ?? $headers['authorization'] ?? '';
    
    if (preg_match('/Bearer\s+(.*)$/i', $authHeader, $matches)) {
        $apiToken = $matches[1];
        
        // Verificar token na base de dados
        $config = require __DIR__ . '/../../app/config/database.php';
        $dsn = $config['dsn'];
        $pdo = new PDO($dsn, $config['username'], $config['password'], $config['options']);
        
        $stmt = $pdo->prepare("
            SELECT us.user_id, u.username, u.email 
            FROM user_sessions us 
            JOIN users u ON us.user_id = u.id 
            WHERE us.api_token = ? AND us.is_active = 1 AND us.expires_at > NOW() AND u.is_active = 1
        ");
        $stmt->execute([$apiToken]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($result) {
            return $result;
        }
    }
    
    return null;
}